                ...
                <div key={index} className="pt-11 pb-8 border-b border-gray-100 max-xl:max-w-2xl max-xl:mx-auto">
                  ...
                  <div className="flex sm:items-center md:items-start flex-col min-[400px]:flex-row justify-between gap-4 mb-4">
                    <div className="flex items-center gap-3">
                      <img src={`${import.meta.env.VITE_CDN_GET_IMAGE}/jvalleyverseImg/${review.user.profile_image_url}`} alt={`${review.user.name} image`} className="w-8 h-8 rounded-full object-cover" />
                      <h6 className="font-semibold text-lg leading-8 text-indigo-600 ">{review.user.name}</h6>
                    </div>
                    <p className="font-normal text-sm text-gray-400">{dayjs(review.created_at).format("DD MMM YYYY HH:mm")}</p>
                  </div>
