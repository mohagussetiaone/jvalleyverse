...
export const Menu = ({ setActive, children }: { setActive: (item: string | null) => void; children: React.ReactNode }) => {
  return (
    <nav onMouseLeave={() => setActive(null)} className="relative w-full boder border-transparent dark:border-white/[0.2] bg-transparent shadow-input space-x-4 py-2 cursor-pointer">
      {children}
    </nav>
  );
};
...
export const NavbarItem = ({ title, description, onClick, src }: NavbarItemProps) => {
  return (
    <div onClick={onClick} className="flex space-x-2 cursor-pointer">
      <img src={src} width={140} height={70} alt={title} className="flex-shrink-0 rounded-md shadow-2xl" />
      <div className="text-start pl-4">
        <h4 className="text-xl font-bold mb-1 text-black dark:text-white">{title}</h4>
        <p className="text-neutral-700 text-start max-w-[10rem] dark:text-neutral-300">{description}</p>
      </div>
    </div>
  );
};
